...
class ExcelWriter:
    ...
    def __init__(
        ...
        additional_dataframes: list[pd.DataFrame],
        fields: list[Field],
        grouping_rule: str,
        report_name: str,
    ):
    ...
    def create_workbook(self, save=True):
        ...
        for worksheet_index, worksheet in enumerate(self.workbook.worksheets):
            self.delete_extra_columns(
                worksheet=worksheet, worksheet_index=worksheet_index
            )
        ...
        if save is True:
            ...
            for file in os.listdir(self.current_output_dir):
                os.rename(
                    os.path.join(self.current_output_dir, file),
                    os.path.join(self.previous_output_dir, file),
                )
            self.workbook.save(
    ...
    def update_workbook(self, key_column: str):
        ...
        # Check if the current output directory is empty. If it is, it means the workbook has not been generated yet for this year.
        if len(os.listdir(self.current_output_dir)) == 0:
            self.create_workbook()
        else:
            # Build a new in-memory workbook to compare against
            self.create_workbook(save=False)
            col_lookup = self.col_lookups[0]
            column_info = col_lookup.get(key_column, None)
            if column_info is None:
                raise ValueError(
                    f"Column '{key_column}' not found in column lookup. {col_lookup}"
                )
            common_column_index = column_info.get("idx", None)

            old_workbook_path = os.listdir(self.current_output_dir)[0]
            old_workbook_path = os.path.join(self.current_output_dir, old_workbook_path)

            # Load the old and new workbooks
            old_workbook = openpyxl.load_workbook(old_workbook_path)
            old_sheet = old_workbook["Main"]
            old_keys = set()
            ...
            for row in old_sheet.iter_rows(min_row=2, values_only=False):
                self.set_cells_needing_embedded_url(row, 0)
                self.set_cells_needing_comment(row, 0)
                self.format_impact_cell(row, 0)

            # Save the updated old workbook
            old_workbook.save(old_workbook_path)
            # old_workbook.save('updated_old_workbook.xlsx')
    ...
    def set_cells_needing_comment(self, row, worksheet_index):
        ...
        for target_column, target_column_info in worksheet_col_lookup.items():
            ...
            cell_needing_comment = self.find_cell_by_column(
                row=row, column_name=target_column, worksheet_index=worksheet_index
            )
            ...
            if target_column_info["comment_field"] != "":
                ...
                cell_with_comment = self.find_cell_by_column(
                    row=row, column_name=comment_column, worksheet_index=worksheet_index
                )
    ...
    def set_cells_needing_embedded_url(self, row, worksheet_index):
        ...
        columns_needing_embedded_url = [
            target_col
            for target_col, target_col_data in col_lookup.items()
            if col_lookup[target_col]["embed_url"] is True
        ]
        for column_name in columns_needing_embedded_url:
            cell_needing_embedded_url = self.find_cell_by_column(
                row, column_name, worksheet_index
            )
            cell_with_embedded_url = self.find_cell_by_column(
                row, "URL", worksheet_index
            )
    ...
    def find_cell_by_column(self, row, column_name: str, worksheet_index: int = 0):
        ...
        target_cell = [cell for col_num, cell in enumerate(row, start=1) if col_num == idx + 1][0]
    ...
    def _create_column_lookups(self):
        ...
        for sheet in self.workbook.worksheets:
            ...
            for col in sheet.iter_cols(1, sheet.max_column):
                ...
                field = next(
                    (field for field in self.fields if field.field_name == column_name),
                    None,
                )
                ...
                col_lookup[column_name] = {
                    "idx": column_idx,
                    "comment_field": comment_field,
                    "dont_return_field": dont_return_field,
                    "embed_url": embed_url,
                }
                column_idx += 1
    ...
    def delete_extra_columns(self, worksheet, worksheet_index):
        ...
        columns_to_delete = [
            column_lookup[col]["comment_field"]
            for col in column_lookup
            if column_lookup[col]["comment_field"] != ""
        ]
    ...
    def get_impact_color(self, impact):
        ...
        green_fill = PatternFill(
            start_color="C6EFCC", end_color="C6EFCC", fill_type="solid"
        )
    ...
    def format_impact_cell(self, row, worksheet_index=0):
        ...
        cell = self.find_cell_by_column(
            row=row, column_name="Impact Level", worksheet_index=worksheet_index
        )
        ...
        if cell_color != None:
            cell.fill = cell_color  # https://stackoverflow.com/questions/30484220/fill-cells-with-colors-using-openpyxl
            cell.font = Font(color=text_color)
    def insert_row(self, data, worksheet, start_row):

        worksheet.insert_rows(start_row)
        for col_num, value in enumerate(data):
            worksheet.cell(row=start_row, column=col_num + 1, value=value)
